...
class SliverReorderableListState extends State<SliverReorderableList> with TickerProviderStateMixin {
    ...
    setState(() {
      if (_dragInfo != null) {
        ...
        for (var element in _items.values) {
          var isTarget = element._isTarget;
          if (isTarget) {
            element._isTarget = false;
            element.rebuild();
          }
        }
        // TODO:
  ...
  void _dragUpdateItems() {
    ...
    for (final _ReorderableItemState item in _items.values) {
      ...
      var bottom = geometry.bottom;
      var v = itemExtent / 2.75;
      if (proxyItemStart < top + v && top + v < proxyItemEnd) {
        item._isTarget = true;
      } else if (proxyItemStart < bottom - v && bottom - v < proxyItemEnd) {
        item._isTarget = true;
      } else {
        item._isTarget = false;
      }
      // TODO:
